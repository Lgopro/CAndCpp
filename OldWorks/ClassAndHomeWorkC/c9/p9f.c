...
/*
Description: Function finds the wanted meeting by hour selected.
Input: pointer to _ad and begin hour
Output: returns 1 if removed meeting and 0 when there is an error.
*/
size_t FindMet(struct AD* _ad, float _begin)
{
	int i;
...
/*
Description: Function free the struct.
Input: pointer to _ad.
Output: NULL.
*/
struct AD* DestroyArray(struct AD* _ad)
{
...
/*
Description: Function removes a meeting by getting the begin hour.
Input: pointer to _ad and begin hour
Output: returns 1 if removed meeting and 0 when there is an error.
*/
ErrorCode RemoveMeeting(struct AD* _ad,float _beginHour)
...
/*
Description: Function swaps between 2 meetings.
Input: pointer to _ad and _i and _j of the 2 meetings.
Output: void.
*/
void Swap(struct AD * _ad, int _i,int _j)
{
...
/*
Description: Function prints all meeting by order.
Input: pointer to _ad.
Output: void.
*/
void PrintAD(struct AD * _ad)
...
/*
Description: Function inserts meeting to the _ad.
Input: pointer to _ad and pointer to meeting.
Output: 1 if the inserts goes by plan and 0 if there is any error.
*/
ErrorCode InsertMeeting(struct AD* _ad, struct meeting *_newMeet)
{
	if(_newMeet==NULL)
...
/*
Description: Function gets the information about the meeting and check if this meeting is in the right time.
Input: pointer to _ad.
Output: 1 if the inserts goes by plan and 0 if there is any error.
*/
...
/*
Description: Function gets the information about the meeting and check if this meeting and structs it.
Input: begin, end time of meeting and room of the meeting.
Output: the struct of the meeting created if the inserts goes by plan and NULL if there is any error.
*/
struct meeting* CreateMet(float _begin, float _end, int _room)
{
	struct meeting* meet;
...
/*
Description: Function gets the information the blocksize and initsize of the _ad and sends it to create the _ad.
Input: pointer to _ad.
Output: the struct of the _ad.
*/
...
/*
Description: Function gets the information the blocksize and initsize of the _ad and create the _ad.
Input: initual size and block size.
Output: the struct of the _ad if it goes by plan and NULL if it dosent.
*/
...
/*
Description: Function gets the hour of meeting.
Input: void.
Output: hour of meeting.
*/
float GetHour(void)
{
	float hour;
...
/*
Description: Function prints all meeting by order to selected file by name.
Input: pointer to _ad.
Output: void.
*/
void StoreAd(struct AD* _ad, char* _filename)
...
{
	char name[50];
	printf("Please enter the name of the file you want to store the meetings to and then add .txt\n");
	fgets(name,50,stdin);
	StoreAd(_ad, name);
...
/*
Description: Function increase the size of the meetings by block size if its full.
Input: pointer to _ad and _m_day.
Output: return pointer to increased size of the meetings.
*/
struct meeting** IncreaseADSize(struct meeting* *_m_day,struct AD* _ad )
{
...
/*
Description: Function reduse the size of the meetings by block size if its full.
Input: pointer to _ad and _m_day.
Output: return pointer to increased size of the meetings.
*/
struct meeting** ReduseADSize(struct meeting* *_m_day,struct AD* _ad )
...
struct AD* LoadAD(char* _filename)
{
	...
	fclose(fp);
	fp=fopen(_filename, "r");
	AD=CreateAD(countlines, DEFAULT_BLOCK_SIZE);
	for(i=0;i<countlines;i++)
	{
		fscanf(fp,"%f %f %d\n", &begin,&end,&room);
		AD->m_day[i]=CreateMet(begin, end, room);
		InsertMeeting(AD, AD->m_day[i]);
	}
