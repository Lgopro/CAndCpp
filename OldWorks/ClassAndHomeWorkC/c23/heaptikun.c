...
static int largest(int _left,int _right, int _father,int _fatherindex)
{
	if(_father >= _right && _father >= _left)
	{
		return _fatherindex;
	}
	if(father < left && right <= left)
	{
		return LEFT(_fatherindex);
	}
	if( RIGHT( _father < _right && _left <= _right)
	{
		return RIGHT(_fatherindex);
	}
}
