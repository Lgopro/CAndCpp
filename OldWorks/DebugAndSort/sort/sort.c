...
static void SwapNum(int _arr[], int _i, int _j);
...
/*static ADTErr CountingSortRadix(Vector* _vec, int _maxValue);*/
...
ADTErr CountingSort(Vector* _vec, int _maxValue)
{
	...
	if(NULL == (arr=(int*)calloc(numberofitems,sizeof(int))))
	{
...
{
	int numberofitems,number,hezka=0,i,j/*,item1,item2*/;
	int* arr;
	int* arrfinal;
	Vector* help;
	if(NULL == _vec)
	...
	help = VectorCreate(numberofitems,numberofitems);
	for(i=0;i<numberofitems;i++)
	{
		VectorGet(_vec, i,&number);
		arr[i]=number;
		arrfinal[i]=number;
	}
	while(hezka!= _nDigits)
	{
		for(i=0;i<numberofitems;i++)
		{
			number=arr[i]%10;
			VectorSet(help,i,number);
		
		}
		number=0;
		CountingSort(help, numberofitems);
		
		/*CountingSort(_vec, numberofitems);
		VectorPrint(_vec);
		for(i=0;i<numberofitems;i++)
    	{
		...
		for(i=0;i<numberofitems;i++)
		{
			VectorGet(help, i,&number);
			for(j=i;j<numberofitems;j++)
			{
				if(arr[j]% 10 == number)
				{
					SwapNum(arr,i, j);
					SwapNum(arrfinal,i,j);
