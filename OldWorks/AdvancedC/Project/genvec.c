...
Vector* Vector_Create(size_t _initialCapacity, size_t _blockSize)
{
	...
	if(_initialCapacity == 0 && _blockSize == 0)
	{
 	...
 	if(NULL == (Vector=(struct Vector*)malloc(sizeof(struct Vector))))
   {
   		return NULL;
   }
...
void Vector_Destroy(Vector** _vector, void (*_elementDestroy)(void* _item))
{
	...
	if(NULL == _vector || NULL == *_vector || (*_vector)->m_magic != MAGIC)
	{
		return;
	}
...
Vector_Result Vector_Append(Vector* _vector, void* _item)
{
	...
	if(_vector->m_nItems == _vector->m_originalSize && _vector-> m_blockSize == 0)
	{
		return VECTOR_OVERFLOW;
	}
	if(_vector->m_nItems == _vector->m_originalSize +_vector->m_blockSize)
	{
		_vector->m_items=(void*)realloc(_vector->m_items, sizeof(void**)*(_vector->m_size+_vector->m_blockSize));
